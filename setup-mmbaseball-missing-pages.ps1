# =============================
# PowerShell Script: setup-mmbaseball-missing-pages.ps1
# Purpose: Creates missing route folders and "Coming Soon" placeholder pages
# =============================

$basePath = "C:\Dev\mm-baseball-clean\src\app"

# Define the pages you want to create
$pages = @(
    @{ name = "book-a-lesson"; title = "Book a Lesson" },
    @{ name = "programs"; title = "Programs" },
    @{ name = "contact"; title = "Contact" }
)

# Shared page content template (interpolates title)
$pageTemplate = @'
"use client";
import { motion } from "framer-motion";

export default function PAGEFUNCNAME() {
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#42B3E0] to-[#2095C6] py-20 px-6 flex flex-col items-center text-center">
      <motion.h1
        className="text-5xl md:text-6xl font-extrabold text-[#0A2351] mb-6 drop-shadow-[0_3px_4px_rgba(10,35,81,0.25)]"
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, ease: "easeOut" }}
      >
        PAGETITLE
      </motion.h1>

      <motion.p
        className="text-[#0A2351]/90 text-lg max-w-xl mx-auto leading-relaxed"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3, duration: 0.8 }}
      >
        This page is currently under construction.
        <br />
        <span className="font-semibold text-[#FDB827]">
          Coming Soon — content for this section will be added soon.
        </span>
      </motion.p>

      <motion.div
        className="mt-16 max-w-md bg-white rounded-xl p-6 shadow-md border-2 border-[#FDB827]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.6, duration: 0.8 }}
      >
        <a
          href="/"
          className="inline-block bg-[#0A2351] text-[#FDB827] font-semibold px-6 py-2 rounded-md shadow-md hover:bg-[#FDB827] hover:text-[#0A2351] hover:shadow-[0_0_15px_#FDB82790] transition-all"
        >
          Back to Home
        </a>
      </motion.div>
    </section>
  );
}
'@

# Create each folder and file
foreach ($page in $pages) {
    $folderPath = Join-Path $basePath $page.name
    $filePath = Join-Path $folderPath "page.tsx"

    # Create folder if it doesn't exist
    if (-not (Test-Path $folderPath)) {
        New-Item -ItemType Directory -Path $folderPath | Out-Null
        Write-Host "Created folder: $folderPath"
    }

    # Customize template with page title and function name
    $content = $pageTemplate.Replace("PAGETITLE", $page.title)
    $content = $content.Replace("PAGEFUNCNAME", ($page.title -replace " ", "") + "Page")

    # Create or overwrite page.tsx file
    Set-Content -Path $filePath -Value $content -Encoding UTF8
    Write-Host "Added file: $filePath"
}

Write-Host "`n✅ All placeholder pages created successfully!"
