...
export default function VideoBreakdownPage() {
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#42B3E0] to-[#2095C6] py-20 px-6 flex flex-col items-center text-center">
      ...
      >
        Game-speed analysis and mechanical insight designed to turn every rep into data-driven progress.
      </motion.p>

      {/* === VIDEO SECTION === */}
      <motion.div
        className="w-full max-w-3xl bg-white rounded-xl shadow-lg overflow-hidden border-2 border-[#FDB827] transition-all hover:shadow-[0_0_25px_rgba(253,184,39,0.5)]"
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <video
          ...
          playsInline
          controls
          poster="/videos/thumbnail.jpg" // optional thumbnail before play
        ...
        <div className="p-6 text-left">
          ...
          <p className="text-[#0A2351] opacity-90 leading-relaxed">
            This swing shows excellent rhythm against 11U true game tempo. The
            load is smooth and compact with a soft, balanced landing that keeps
            the head quiet through launch. The barrel enters the zone early and
            stays on plane for roughly 17–18 inches of path length — ideal for
            adjustability. Sequencing efficiency sits around 94–95 % of
            top-velocity swings, producing bat speed near 64 mph and an
            estimated exit velocity of 63 mph. Launch angle comes off 13–15°,
            carrying roughly 155–165 ft.
          </p>

          {/* Stat Bar */}
          <div className="mt-4 flex justify-between text-sm text-[#0A2351]/80 font-semibold">
            <span>Bat Speed: 64 mph</span>
            <span>Exit Velo: 63 mph</span>
            <span>Launch: 14°</span>
            <span>Carry: 160 ft</span>
          </div>
        </div>
      </motion.div>
